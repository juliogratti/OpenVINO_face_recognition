...
nome_foto = "subject_18" #"subject02.wink"


for parametro in sys.argv:
    nome_foto = str(parametro)


cap = cv2.VideoCapture("imagesTeste/originalImages/"+nome_foto+".jpg")               
...
while Count < 1:
    ...
    if np.average(gray) > 110:                                                                      # Testing the brightness of the image
        ...
        for (x, y, w, h) in faces:                                                                  # Frames  LOCATION X, Y  WIDTH, HEIGHT
            ...
            cv2.imwrite("data/" + nome_foto+ ".jpg", frame)
            cv2.waitKey(200)
